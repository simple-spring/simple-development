...
import static com.chexin.simple.development.support.ResponseCode.RES_DECRYPT_FAIL;
import static com.chexin.simple.development.support.ResponseCode.RES_ENCRYPT_FAIL;
...
public class AESUtils {
    ...
    public static String decrypt(String sKey, String sSrc){
        try {
            ...
            if (StringUtils.isBlank(sKey)) {
                throw new GlobalException(RES_DECRYPT_FAIL, "Key为空null");
            }
            // 判断Key是否为16位
            if (sKey.length() != 16) {
                throw new GlobalException(RES_DECRYPT_FAIL, "Key长度不是16位");
            }
            ...
            throw new GlobalException(RES_DECRYPT_FAIL, "解密失败");
        }
    }
    ...
    public static String encrypt(String sKey, String sSrc){
        if (StringUtils.isBlank(sKey)) {
            throw new GlobalException(RES_ENCRYPT_FAIL, "Key为空null");
        }
        // 判断Key是否为16位
        if (sKey.length() != 16) {
            throw new GlobalException(RES_ENCRYPT_FAIL, "Key长度不是16位");
        }
        try {
            byte[] raw = sKey.getBytes("utf-8");
            SecretKeySpec skeySpec = new SecretKeySpec(raw, "AES");
            ...
            logger.error(DateUtils.getCurrentTime(), ex);
            throw new GlobalException(RES_ENCRYPT_FAIL, "加密失败失败");
        }
    }
}
