 ...
 */
package com.chexin.simple.development.support.utils.qrCode;
...
//        	bitMatrix = deleteWhite(bitMatrix);
     ...
     * @param params
     */
    public static void generateQRImageOutput(QRCodeParams params,OutputStream outputStream){

        initData(params);
        if(params.getTxt() == null) {
        ...
        try {
            MatrixToImageConfig config = new MatrixToImageConfig(onColor, offColor);
            BitMatrix bitMatrix = new MultiFormatWriter().encode(params.getTxt(),BarcodeFormat.QR_CODE, width, height, hints);
            MatrixToImageWriter.writeToStream(bitMatrix,"jpg",outputStream,config);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
